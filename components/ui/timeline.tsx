...
export const Timeline = ({ data, title, description, children }: { data: TimelineEntry[], title?: string, description?: string, children?: React.ReactNode }) => {
  ...
  return (
    <div
      className="w-full bg-background font-serif md:px-10"
      ref={containerRef}
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        ...
        <p className="text-foreground/80 text-lg md:text-xl max-w-sm">
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-background flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-foreground border border-foreground p-2" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl text-foreground/50 ">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left text-foreground/50">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
